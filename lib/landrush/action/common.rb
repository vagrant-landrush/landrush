    ...
    module Common
      ...
      def provider
        provider_name = SUPPORTED_PROVIDERS.fetch(machine.provider.class.name) do |key|
          fail "The landrush plugin does not support the #{key} provider yet!"
        end

        if provider_name == :parallels && Gem::Version.new(VagrantPlugins::Parallels::VERSION) < Gem::Version.new("1.0.3")
          fail "The landrush plugin supports the Parallels provider v1.0.3 and later. Please, update your 'vagrant-parallels' plugin."
        end
